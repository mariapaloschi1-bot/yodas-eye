    ...
    <div class="max-w-5xl mx-auto p-6 lg:p-12 font-sans">
      <div class="bg-slate-800 rounded-3xl shadow-2xl border border-slate-700 overflow-hidden relative">
        ...
        <div class="p-8 space-y-10">
          ...
          <!-- API Key -->
          <div>
            <label class="block text-xs font-bold text-teal-500 uppercase tracking-widest mb-2">
               Chiave d'Accesso (Gemini API Key)
            </label>
            <div class="relative group">
              <input 
                [(ngModel)]="apiKey" 
                type="password"
                placeholder="La chiave segreta qui inserire devi (AIzaSy...)..."
                class="w-full p-4 pl-12 bg-slate-900 border border-slate-700 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition text-slate-200 shadow-inner group-hover:border-slate-600"
              />
            ...
            <p class="text-xs text-slate-500 mt-2 flex items-center gap-1">
               <svg class="w-3 h-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
               Input ottimizzato per stabilità.
            </p>
          </div>
...
export class InputViewComponent {
  ...
  isValid() {
    return this.apiKey && this.focusBrandName && this.focusBrandData && 
