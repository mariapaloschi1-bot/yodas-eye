...
import { TabMatrixComponent } from './tab-matrix.component';
      ...
      <main class="flex-1 overflow-auto p-4 md:p-6 custom-scrollbar relative">
        <div class="max-w-[1600px] mx-auto pb-12">
          @switch (activeTab()) {
            ...
            @case ('clusters') {
              @if (data.tab2_theme_clustering && data.meta) {
                <app-tab-clusters [data]="data.tab2_theme_clustering" [meta]="data.meta" />
              } @else {
                <div class="text-center text-slate-500 py-10">Dati cluster non disponibili.</div>
              }
            }
            @case ('matrix') {
              @if (data.tab3_dual_clustering) {
                <app-tab-matrix [data]="data.tab3_dual_clustering" />
              } @else {
                <div class="text-center text-slate-500 py-10">Matrice non disponibile.</div>
              }
            }
            @case ('depth') {
